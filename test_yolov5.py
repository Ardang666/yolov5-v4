...
def detect(save_img=False):
    ...
    if opt.cfg == 'qua':
        ...
        if opt.jit:
            print("### jit")
            ct.save_as_cambricon('yolov5s_int8_' + str(batchsize) + 'b_' + str(corenum)  + 'c')
            torch.set_grad_enabled(False)
            ct.set_core_number(corenum)
            trace_input = torch.randn(batchsize, 3, 384, 640, dtype=torch.float)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if opt.cfg == 'qua':
            ...
            img = np.repeat(img, batchsize, axis=0)
